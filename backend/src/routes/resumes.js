...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!resume.currentVersionId) {
      throw ApiError.badRequest("This resume has no current version to analyze");
    }
    const version = await loadVersion(resume._id, resume.currentVersionId);

    const result = {
      atsScore: 78,
      scoreBreakdown: { formatting: 20, keywords: 18, impact: 22, clarity: 18 },
      issues: [
        {
          title: "Missing quantifiable metrics in bullet points",
          severity: "medium",
          explanation:
            "Bullet points describe duties but don't show measurable impact.",
          fix: "Add numbers, percentages, or outcomes to key achievements.",
        },
        {
          title: "No dedicated skills section keywords match",
          severity: "low",
          explanation:
            "Some role-relevant keywords are missing from the skills section.",
          fix: "Add missing keywords relevant to the target role.",
        },
      ],
      strengths: [
        {
          title: "Clear work experience structure",
          evidence: "Roles and dates are well organized.",
        },
        {
          title: "Consistent formatting",
          evidence: "Consistent font and spacing throughout.",
        },
      ],
      bulletRewrites: [],
      keywordsPresent: ["JavaScript", "React", "Node.js"],
      keywordsMissing: ["TypeScript", "AWS"],
      summary:
        "Solid resume with room to strengthen quantifiable impact statements.",
    };
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const result = await analyzeResume(version.rawText, req.body.targetRole);
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!analysis) throw ApiError.notFound("No analysis found for this version");
